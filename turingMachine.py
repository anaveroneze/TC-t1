...
class TuringMachine:
    ...
    def run(self, w):
        ...
        c = 0
        #Percorre toda a entrada até que o estado atual seja igual ao estado final
        while(self.estadoAtual != self.estadoFinal):
            ...
            for d in range(self.numTransicoes):
                ...
                if(self.transEntrada[d][0] == self.estadoAtual):  #Se o estado atual é o mesmo estado da proxima transição, verifica ela
                    for e, f in enumerate(self.fitas):  #Para todas as fitas
                        print("fita", e, "=", f.lerFita())
                        if(f.lerFita() != self.transEntrada[d][1][e]):  #Se o simbolo lido na transição é diferente do simbolo da fita, vai pra proxima transição
                            #print(f.lerFita(), self.transEntrada[d][1][e], f.posicao)
    ...
    def limpaFita(self):
        self.fitas = []
        for i in range(self.numFitas):
            self.fitas += [Fita(branco=self.alfFita[-1])]
